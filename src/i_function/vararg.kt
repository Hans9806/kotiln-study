...
// 가변인자는 매개변수 어디에나 올 수 있지만, 가변인자 뒤에 매개변수가 있으면
//반드시 매개변수 이름을 지정하여 호출해야 함
fun printNumberWithSuffix(vararg numbers: Int, suffix: String) {
    // 같은 인자로 받음
    for (number in numbers) {
        print("$number$suffix")
    }
...
fun main() {
    ...
    printNumberWithSuffix(1,2,3, suffix = "개")

    // 가변인자 뒤에 다른 매개변수가 있으면
    // 반드시 매개변수 이름을 지정하여 호출해야 함
    printNumberWithSuffix(1,2,3, suffix = "개")
    printNumberWithSuffix(4,5,6, suffix = "자루")
    ...
    printNumbers(*ints)
    printNumberWithSuffix(*ints, suffix = "통")
}